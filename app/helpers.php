...
if (!function_exists('currencyAmountShort')) {
    ...
    function currencyAmountShort($amount)
    {
        // You can change this to the appropriate exchange rate if needed
        $exchangeRate = 1; // 1 USD to IDR

        // Convert the amount to IDR
        $amountInIDR = $amount * $exchangeRate;

        $units = ['', 'rb', 'jt', 'M', 'T'];
        for ($i = 0; $amountInIDR >= 1000; $i++) {
            $amountInIDR /= 1000;
        }

        return round($amountInIDR, 2) . ' ' . $units[$i]; // Adjust the precision and add the unit
    }
}
...
if (!function_exists('updateMap')) {
    function updateMap($data)
    {
        ...
        if ($location) {
            ...
            $data->update([
                'address' => $address,
                // 'address_detail' => array_key_exists("address_detail", $location) ? $location['address_detail'] : '',
